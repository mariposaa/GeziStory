...
# OpenWeatherMap API Key (Demo Key)
WEATHER_API_KEY = "4a3503250b73c46d3d44101e63777507" 

st.set_page_config(page_title="GeziStory", page_icon="🧿", layout="centered")

# --- 2. HTML VE TASARIM YARDIMCILARI (ASKERİ DÜZEN) ---
# HTML kodları sola dayalıdır (Sıfır Girinti).
...
/* KEŞFET: Post Kartı Tasarımı (Dikey) */
.discover-card {
...
.card-caption {
padding: 10px 12px; font-size: 12px; color: #444; line-height: 1.4;
...
/* ROTALAR: Gelişmiş Yatay Kart Tasarımı */
...
/* Galeri Başlığı (Collage Layout) */
.route-header-collage {
display: flex; height: 220px; width: 100%; position: relative;
}
.collage-left {
...
padding: 20px 15px 12px 15px; color: white;
}
...
padding-bottom: 10px; gap: 0px; /* Oklar için gap'i kapattık */
}
.timeline-step {
...
.timeline-arrow { 
color: #bbb; font-size: 18px; margin: 0 5px; 
display: flex; align-items: center; height: 100%;
}
/* Rota Özet Kartı (Grid İçin) */
.route-card-summary {
...
/* Kahve Ismarla Butonu (Gold/Premium) */
.coffee-btn-container {
margin-top: 20px; padding-top: 15px; border-top: 1px dashed #ddd; text-align: center;
}
.stButton button[kind="primary"] { /* Streamlit Primary Button Override */
background-color: #FFD700 !important; color: #333 !important; border: none !important; font-weight: bold !important;
box-shadow: 0 4px 10px rgba(255, 215, 0, 0.4) !important;
}
.stButton button[kind="primary"]:hover {
background-color: #E6C200 !important; transform: scale(1.02);
}
...
def get_route_card_html(story):
    ...
    if not images and story.get('img'):
        images = [story['img']] 
...
class FirebaseService:
    ...
    def get_profile(self, uid):
        ...
        return {"nick": "Misafir", "balance": 0, "earnings": 0}
        ...
        try:
            ...
            for doc in r.json()['documents']:
                try:
                    ...
                    liked_by_me = False
                    if st.session_state.user_uid and st.session_state.user_uid in liked_uids:
                        liked_by_me = True
    ...
    def update_interaction(self, story_id, action, current_likes=[], current_views=0, comment_data=None, current_comments=[]):
        ...
        if action == "like" and st.session_state.user_uid:
            ...
            new_comment_obj = {
                "mapValue": {
                    "fields": {
                        "user": {"stringValue": comment_data['user']},
                        "text": {"stringValue": comment_data['text']},
                        "date": {"stringValue": str(datetime.now())}
                    }
                }
            }
            existing_comments_json = []
            for c in current_comments:
                existing_comments_json.append({
                    "mapValue": {
                        "fields": {
                            "user": {"stringValue": c['user']},
                            "text": {"stringValue": c['text']},
                            "date": {"stringValue": c['date']}
                        }
                    }
                })
...
def render_route_detail_content(story, fb_service): 
    ...
    if c_like.button(f"{like_icon} {story['like_count']}", key=f"d_lk_{story['id']}"):
        if st.session_state.user_token: 
            fb_service.update_interaction(story['id'], "like", current_likes=story.get('liked_uids', []))
            st.rerun()
    ...
    # YENİ KONTROL: Rota Sahibi Kendisi ise Butonu Gizle
        ...
        if st.button("☕ Yazara Bir Kahve Ismarla (20 TL)", type="primary", use_container_width=True, key=f"tip_{story['id']}"):
            if st.session_state.user_token:
                ...
                if current_user_balance >= 20:
                    ...
                    if success:
                        st.balloons()
                        st.success("Harikasın! Kahve ısmarlandı. ☕")
                        st.session_state.user_balance -= 20
                    else:
                        st.error(f"İşlem başarısız: {msg}")
                else:
                    st.warning("⚠️ Bakiyeniz yetersiz.")
                    st.info("📱 Cüzdanına para yüklemek için lütfen mobil uygulamamızı kullan.")
            else:
                st.warning("Kahve ısmarlamak için giriş yapmalısın.")
    else:
        st.info("😏 Bu senin kendi rotan. Kendine kahve ısmarlayamazsın (ama fikir güzel).")
        
    st.markdown('</div>', unsafe_allow_html=True)

if dialog_decorator:
    @dialog_decorator("💬 Yorumlar")
    def view_comments_dialog(story, fb_service):
        render_comments_content(story, fb_service)
        
    @dialog_decorator("🗺️ Rota Detayları")
    def view_route_detail_dialog(story, fb_service):
        render_route_detail_content(story, fb_service)
else:
...
def render_create_route_section(fb_service):
    if 'new_route_stops' not in st.session_state:
    ...
    if c4.button("Ekle", type="secondary", use_container_width=True):
        if s_name:
            st.session_state.new_route_stops.append({
                "place": s_name, "type": s_type, "price": s_price, "time": "00:00"
            })
            st.rerun()
        else:
            st.toast("Lütfen durak adı girin.")
    ...
    if st.session_state.new_route_stops:
        ...
        if st.button("Son Durağı Sil"):
            st.session_state.new_route_stops.pop()
            st.rerun()
    ...
    if st.button("Rotayı Yayınla", type="primary", use_container_width=True):
        if not img_file: st.error("Lütfen bir kapak fotoğrafı yükleyin.")
            ...
            with st.spinner("Rota oluşturuluyor..."):
                ...
                if img_url:
                    total_budget = sum([s['price'] for s in st.session_state.new_route_stops])
                    fb_service.add_story({
                        "title": title, "city": city, "img": img_url, 
                        "summary": summary, "category": category,
                        "budget": total_budget, 
                        "stops": st.session_state.new_route_stops,
                        "author": st.session_state.user_nick, 
                        "uid": st.session_state.user_uid
                    })
                    st.session_state.new_route_stops = []
                    st.session_state.show_create_form = False # Formu Kapat
                    st.success("Harika! Rota başarıyla yayınlandı. 🎉")
                    time.sleep(1.5)
                    st.rerun()
...
def render_rotalar_page(stories, fb_service): 
    ...
    # 1. Rota Oluşturma Alanı (AÇ/KAPA MEKANİZMASI)
    if st.session_state.user_token:
        # Session state'de formun açık olup olmadığını tut
        if 'show_create_form' not in st.session_state:
            st.session_state.show_create_form = False

        if not st.session_state.show_create_form:
            if st.button("➕ Yeni Rota Ekle", use_container_width=True):
                st.session_state.show_create_form = True
                st.rerun()
        else:
            # Form Açıkken Vazgeç Butonu
            if st.button("❌ Vazgeç", type="secondary"):
                st.session_state.show_create_form = False
                st.rerun()
            
            # Formu Render Et
            with st.container(border=True):
                render_create_route_section(fb_service)
    ...
    demo_route = {
        ...
        "stops": [
            {"place": "Ayasofya Camii", "time": "09:00", "price": 0, "type": "Tarih"},
            {"place": "Sultanahmet", "time": "11:00", "price": 0, "type": "Manzara"},
            {"place": "Köfteci", "time": "13:00", "price": 350, "type": "Yemek"},
            {"place": "Gülhane", "time": "15:00", "price": 0, "type": "Doğa"},
            {"place": "Eminönü", "time": "17:00", "price": 100, "type": "Manzara"}
        ],
        "uid": "demo_user_uid", 
        "like_count": 128,
        "liked_by_me": False,
...
def main():
    ...
    if col1.button("🎲 KEŞFET", type="primary" if st.session_state.active_tab == "kesfet" else "secondary", use_container_width=True): st.session_state.active_tab = "kesfet"; st.rerun()
    if col2.button("🗺️ ROTALAR", type="primary" if st.session_state.active_tab == "rotalar" else "secondary", use_container_width=True): st.session_state.active_tab = "rotalar"; st.rerun()
        
    stories = fb.get_stories()
    if st.session_state.active_tab == "kesfet": render_kesfet_page(stories, fb)
    else: render_rotalar_page(stories, fb) # fb parametresi eklendi
    render_sidebar(fb)
